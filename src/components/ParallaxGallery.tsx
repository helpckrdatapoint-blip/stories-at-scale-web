...
const ParallaxGallery = () => {
  ...
  return (
    <div className="w-full bg-[#0a0a0a] relative overflow-hidden">
      ...
      <div
        ref={gallery}
        className="relative box-border flex items-center justify-center h-[250vh] gap-[2vw] overflow-hidden p-[2vw] z-10"
      >
        <Column videos={videos.slice(0, 4)} y={y} />
        <Column videos={videos.slice(3, 7)} y={y2} />
      </div>
...
const Column = ({ videos, y }) => { 
  return (
    <motion.div
      className="relative -top-[45%] flex h-full w-2/6 min-w-[300px] flex-col gap-[2vw] first:top-[-45%] [&:nth-child(2)]:top-[-95%] [&:nth-child(3)]:top-[-45%] [&:nth-child(4)]:top-[-75%]"
      style={{ y }}
    >
      {videos.map((src, i) => ( 
        <div key={i} className="group relative h-[70vh] w-full overflow-hidden rounded-2xl border border-white/10 bg-neutral-900">
          <video
            src={src}
            autoPlay 
            loop     
